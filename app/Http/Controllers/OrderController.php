...
class OrderController extends Controller
{
    ...
    public function createOrder(Request $request)
    {
        ...
        $validatedData['order_date'] =  $request->date; //Carbon::now()->format('Y-m-d');
        ...
        $validatedData['total'] = Cart::total()- (int)(Cart::total()*$request->discount/100);
        $validatedData['discount'] = (int)Cart::total()*$request->discount/100;
        $validatedData['due'] =  (int)(Cart::total()- (Cart::total()*$request->discount/100)) - (int)$validatedData['pay'];
    ...
    public function downloadInvoice(Int $order_id)
    {
        ...
        $due = Order::where('customer_id', $order->customer_id)->sum('due');
        $orderDetails = OrderDetails::with('product')
                        ->where('order_id', $order_id)
                        ->orderBy('id', 'DESC')
                        ->get();
